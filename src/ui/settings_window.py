...
from ui.fixed_point_settings_view import FixedPointSettingsView
from ui.floating_point_settings_view import FloatingPointSettingsView
import numpy
...
class SettingsWindow:
    def __init__(self, root, pos_x, pos_y, column_settings, title, on_closed):
        ...
        self._window.geometry(f'320x200+{pos_x}+{pos_y}')
        ...
        if column_settings.uses_floats():
            self._curr_view = FloatingPointSettingsView(self._window, column_settings)
        else:
            self._curr_view = FixedPointSettingsView(self._window, column_settings)

    def _init_repr_select(self):
        mode_label = tkinter.Label(master=self._window, text='Sisäinen esitysmuoto')
        mode_label.grid(row=0, column=0, rowspan=2, padx=(20, 0), sticky=tkinter.W)
        ...
        float_button.grid(row=0, column=1, padx=(0, 20), pady=(10, 0), sticky=tkinter.E)
        fixed_button.grid(row=1, column=1, padx=(0, 20), pady=(0, 10), sticky=tkinter.E)
    ...
    def _on_mode_changed(self, name, index, mode):
        ...
        if isinstance(self._curr_view, FloatingPointSettingsView):
            self._column_settings.enable_fixed_mode()
            self._curr_view = FixedPointSettingsView(self._window, self._column_settings)
        else:
            self._column_settings.enable_float_mode()
            self._curr_view = FloatingPointSettingsView(self._window, self._column_settings)
